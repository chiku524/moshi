import { showConnectModal, showSignModal } from "../lib/features/walletSlice";
        ...
        <section>
            <div id="connect-modal" className='bg-slate-900 bg-opacity-70 w-screen h-screen absolute top-0 left-0 flex justify-center items-center'>
                ...
                {connectors.map((connector:any) => 
                    <div key={connector.uid} className={'hover:cursor-pointer hover:bg-green-400 rounded p-2'} onClick={async () => {await connect({connector}); dispatch(showSignModal(true)); dispatch(showConnectModal(false))}}>
                        <span>
                            {connector.name}
                        </span>
                    </div>)}
